...
def get_sideline(req):
    ...
    if is_login(req,sessionid):
        ...
        typeid = is_int(typeid)                                 #判断typeid是否为非数字
        if sideline_id == "error" or typeid =="error":                      #如果uid非数字，返回错误信息
        ...
        if(sideline_xiaohao_panduan(uid,sideline_id,sideline_list.number_of_today_sideline,typeid=typeid) == 0):
            #typeid为1为正常副业，typeid为2为教育
            if typeid == 1:
                ...
                if sideline.exists():
                    ...
                    if(len_1 == len_2 == len_3  and (c_type!= None)):
                        ...
                        record.sideline_day_c = today
                        record.sideline_day = d
                ...
                #从数据库中获取相关数据
                getdbname = SkillName.objects.filter(big_id=sideline_id).first()
                userskill = UserBigSkill.objects.get_or_create(user=user)[0]
                dbname = getdbname.db_big_name
                dbname_full = 'userskill.'+dbname
                dbname_of_wuxing = dbname + '_wuxing'
                dbname_of_wuxing_full = 'userskill.'+dbname_of_wuxing
                dbname_of_level = dbname + '_level'
                dbname_of_level_full = 'userskill.'+dbname_of_level
                name = getdbname.big_name
                skillnum = eval(dbname_full)
                skilllevel = eval(dbname_of_level_full)
                skillwuxing = eval(dbname_of_wuxing_full)
                #如果没有大类技能，不能进行教育
                if not skillnum:
                    meg = "您还没有{0}技能，无法进行{0}教育".format(name)
                    result = {
                        "status":status,
                        "message":meg,
                        "data":data
                    ...
                    return HttpResponse(json.dumps(result), content_type="application/json")
                #顺序：先计算突破，再增加技能，最后计算悟性增加
                #1、突破计算
                ispromoted = eureka(skillnum,skilllevel,skillwuxing)
                if ispromoted:      #升级则操作数据库使数据+1
                    exec(dbname_of_level_full+'='+'int'+'('+dbname_of_level_full+')'+'+1')
                #2、增加技能，教育的typebuff为0.9，数据库操作在下方
                ...
                userskill.save()
                #四维消耗
                sideline_xiaohao(uid,sideline_id,sideline_list.number_of_today_sideline,typeid=typeid)
                #统计
                d=datetime.datetime.now().strftime('%Y-%m-%d')
                today=(datetime.datetime.strptime(d,"%Y-%m-%d")-datetime.datetime.strptime('2021-6-3',"%Y-%m-%d")).days
                record = sideline_record.objects.get(uid=uid)
                ...
                data = {
                    'sideline_name':'教育',
                    'skill_id':sideline_id,
                    'skill_name':name,
                    'skill':new_bigskill,
                    'level':eval(dbname_of_level_full),
                    'comprehension':eval(dbname_of_wuxing_full),
                    'eureka':ispromoted,
                }
            else:
                meg='typeid错误'
...
def sideline_skill_increase(uid,big_skill_id,mini_skill_id,type_buff):
    ...
    userskill = UserBigSkill.objects.get_or_create(user_id=uid)[0]
    #获得主技能数值
    bigskill = eval('userskill.' + big_skill_name)
    #获得主技能门槛dbname
    big_skill_level_name = big_skill_name + '_level'
    #获得主技能门槛数值
    big_skill_level = eval('userskill.' + big_skill_level_name)
    #获取
    if mini_skill_id != 0:
        #获得小技能 dbname
...
def sideline_xiaohao_panduan(uid,sideline_id,item,typeid=1):
    ...
    if not sideline or typeid == 2 or not len(sideline.sideline_happy):
        happy = 3
        healthy = 3
        energy = 15
        hunger = 4
    else:
        happy = float(sideline.sideline_happy)
        healthy = float(sideline.sideline_health)
        energy = float(sideline.sideline_energy)
        hunger = float(sideline.sideline_hunger)
    need_happy = happy*item
    need_healthy = healthy*item
    need_energy = energy*item
    need_hunger = hunger*item
...
def sideline_xiaohao(uid,sideline_id,item,typeid=1):
    #次数修正，因为先消耗四维才记录副业次数，导致副业次数始终少1
    item += 1
    getuserstatus = personal_attributes.objects.filter(uid=uid).first()
    #获得副业句柄
